...
Features persistent API key retention and auto-loading from secrets.
"""
...
# Sidebar Controls & Persistent Key Handling
st.sidebar.header("Configuration")

# 1. Check if key exists in Streamlit Secrets (Method 1)
default_key = ""
if "GEMINI_API_KEY" in st.secrets:
    default_key = st.secrets["GEMINI_API_KEY"]

# 2. Persist manual key entry across sessions (Method 2)
...
api_key = st.sidebar.text_input(
    ...
    type="password",
    help="Key is preserved across interactions. Enter once or configure in Streamlit Secrets."
)

# Keep the session state updated if edited
        ...
        if not effective_key:
            ...
            with st.spinner("Querying grounded evidence base..."):
                try:
                    ...
                    st.error(f"Query Error: {str(ex)}")    else:
        with st.spinner("Analyzing text and verifying quotes against raw files..."):
            try:
                files_dict = {f.name: f.read().decode("utf-8") for f in uploaded_files}
